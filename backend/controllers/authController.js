...
const forgotPassword = async (req, res, next) => {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `Password reset email initiated for ${user.email}. Check Ethereal if testing.`
      );
      res
        .status(200)
...
const resetPassword = async (req, res, next) => {
  ...
  console.log(
    "[RESET_PASSWORD_CONTROLLER] Received raw token from params:",
    resetTokenFromParams
  );
  console.log(
    "[RESET_PASSWORD_CONTROLLER] Hashed token for DB lookup:",
    hashedToken
  );
  ...
  try {
    ...
    console.log(
      "[RESET_PASSWORD_CONTROLLER] User found by token:",
      user ? user.email : "No user found or token expired"
    );
    ...
    console.log(
      "[RESET_PASSWORD_CONTROLLER] Password has been reset successfully for user:",
      user.email
    );
    ...
    console.error(
      "[RESET_PASSWORD_CONTROLLER] CATCH BLOCK: Error during password reset:",
      error.message,
      error.stack
    );
    next(error);
...
const googleCallback = (req, res) => {
  ...
  const userString = encodeURIComponent(JSON.stringify(userForFrontend));

  res.redirect(
    `${process.env.FRONTEND_URL}/auth/callback?token=${token}&user=${userString}`
  );
};
